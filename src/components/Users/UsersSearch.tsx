  ...
  return (
    <>
      ...
      {!fetch && searchData.length === 0 && (
        <Section>
          <SearchInput path="/dashboard/search_user" placeholder="Search" />
          ...
          <Table columns={tableColumns}>
            {searchData.map((user) => (
              <TR key={user.id}>
                ...
                <TD>{user.email}</TD>
                <TD>
                  {distanceInWordsStrict(parseISO(user.created_at), Date.now())}
                </TD>
                <TD>
                  {distanceInWordsStrict(parseISO(user.updated_at), Date.now())}
                </TD>
                <TD>
                  <Link
